...
class Inventory(object):
    ...
    @classmethod
    def _load_opts(cls, host):
        return cls._load_password(host)

    @classmethod
    def _load_password(cls, item):
        op_item = item.get("op_item")
        op_account = item.get("op_account")
        if op_item:
            op_item = OP.get_item(op_item, op_account)
            if 'password' in op_item:
                item["password"] = op_item["password"]
                del item["op_item"]
                del item["op_account"]
            else:
                raise ValueError(f"Failed to find password for op_item {op_item} op_account {op_account}")
        return item
